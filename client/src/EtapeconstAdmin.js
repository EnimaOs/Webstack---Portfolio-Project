...
const[prov,setProv]=useState('');
  ...
  function fetchPlans() {
    axios.post('http://localhost:5000/api/plans')
      .then(response => {
  ...
  const handlePlanChange = (e) => {
    ...
    if (selectedPlanValue !== '') {
      axios.post('http://localhost:5000/filtrerparplanconst', { idPlan: selectedPlanValue })
        .then(response => {
  ...
  function charger() {
    axios.post('http://localhost:5000/etapeconstruction')
      .then(response => {
        setMessage(response.data.results);
  ...
  const handleDelete = (id,idetat) => {
    if(idetat<5){
    axios.post('http://localhost:5000/api/modifieretapeconst', {id:id,idetat:idetat})
      .then(response => {
    ...
    <table className="table-auto w-full">
      <thead className="bg-gray-200">
        ...
        <th className="px-4 py-2">Direction Provinciale</th>
          ...
          <th className="px-4 py-2">Montant Estimé</th>
          <th className="px-4 py-2">Partenariats</th>
          <th className="px-4 py-2">budget_partenaire</th>
      ...
      <tbody>
        {message && message.map((rowData, index) => (
          <tr className={`text-center ${index % 2 === 0 ? 'bg-gray-100' : 'bg-gray-50'}`} key={index}>
            ...
            <td className="px-4 py-2">{rowData.montant}</td>
            <td className="px-4 py-2">{rowData.partenaire}</td>
            <td className="px-4 py-2">{rowData.budget}</td>
