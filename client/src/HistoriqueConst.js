...
function HistoriqueConst() {
  ...
  useEffect(() => {
    async function fetchHistorique() {
      try {
        const response = await axios.post("http://localhost:3011/api/HistoriqueCst", { id });
  ...
  const handleView = async (idetape) => {
    try {
      const response = await axios.post("http://localhost:3011/api/HistoriqueEtatCst", { idetat: idetape });
...
export default HistoriqueConst;
