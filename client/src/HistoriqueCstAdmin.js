...
function HistoriqueCstAdmin() {
  ...
  useEffect(() => {
    async function fetchHistorique() {
      try {
        const response = await axios.post("http://localhost:3011/api/HistoriqueAdminCst", { id });
  ...
  const handleView = async (idetape) => {
    try {
      const response = await axios.post("http://localhost:3011/api/HistoriqueAdminEtatCst", { idetat: idetape });
