...
function HistoriqueExAdmin() {
  ...
  useEffect(() => {
    async function fetchHistorique() {
      try {
        const response = await axios.post("http://localhost:5000/api/HistoriqueAdminExt", { id });
  ...
  const handleView = async (idetape) => {
    try {
      const response = await axios.post("http://localhost:5000/api/HistoriqueAdminEtatExt", { idetat: idetape });
