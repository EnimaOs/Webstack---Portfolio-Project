...
function HistoriqueExt() {
  ...
  useEffect(() => {
    async function fetchHistorique() {
      try {
        const response = await axios.post("http://localhost:5000/api/HistoriqueExt", { id });
  ...
  const handleView = async (idetape) => {
    try {
      const response = await axios.post("http://localhost:5000/api/HistoriqueEtatExt", { idetat: idetape });
  ...
  return (
    <>
      <div>
        <h1 className="text-3xl font-bold text-center text-lime-600 relative top-16 ml-20">Historique des étapes d'extension</h1>
      </div>
        ...
        <div className="fixed inset-y-0 left-0 bg-green-900 w-64 p-4">
          <div className="text-white">
            ...
            <nav>
              <ul>
                <li className="mb-4">
                  <Link to="/Home" className="text-white font-medium hover:bg-green-600 py-1 rounded-sm px-2 text-lg flex">
                ...
                <li className="mb-3">
                  ...
                  {showDropdown && (
                    <div className="absolute mt-1 w-64 bg-green-300 rounded-lg shadow-lg">
                      <ul>
                        <li>
                          <Link to="/Acquisition" className="text-black font-medium block px-4 py-2 hover:bg-green-400">Extension</Link>
                        </li>
                        <li>
                          <Link to="/Etudes" className="text-black font-medium block px-4 py-2 hover:bg-green-400">Construction</Link>
                        </li>
                      </ul>
                    </div>
                  )}
                ...
                <li className="mb-3">
                  <Link to="/" className="text-white hover:bg-green-600 font-medium py-1 px-2 rounded-sm text-lg flex">
                    <LogoutIcon className="h-5 w-5 mr-2 mt-1" />Déconnexion
                  </Link>
                </li>
              </ul>
            </nav>
