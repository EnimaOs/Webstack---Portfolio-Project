...
function HistoriqueExt() {
  ...
  useEffect(() => {
    async function fetchHistorique() {
      try {
        const response = await axios.post("http://localhost:3011/api/HistoriqueExt", { id });
  ...
  const handleView = async (idetape) => {
    try {
      const response = await axios.post("http://localhost:3011/api/HistoriqueEtatExt", { idetat: idetape });
