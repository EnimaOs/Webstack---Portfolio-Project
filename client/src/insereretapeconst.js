...
function Insereretapeconst() {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            try {
                const response = await axios.post("http://localhost:3011/charger");
    ...
    useEffect(() => {
        const fetchPlan = async () => {
            try {
                const response = await axios.post('http://localhost:3011/plan');
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post("http://localhost:3011/inserer", { Data: formData, project_id: selectedProject, id_plan: plaan, etape: constr });
                ...
                <div className="mb-4">
                    ...
                    <select className="border border-gray-300 rounded-md py-2 px-4 w-full" id="projectSelect" value={selectedProject} onChange={handleProjectChange}>
                        ...
                        {message && message.map((rowData, index) => (
                            <option key={index} value={rowData.id_projet_constr}>
                                {rowData.id_projet_constr} {rowData.nom_projet} {rowData.date_creation}
                            </option>
                        ))}
                    </select>
                </div>
                ...
                <div className="flex justify-center items-center h-5/6">
                    <form onSubmit={handleFormSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 w-8/12">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                <label className="block text-gray-700" htmlFor="id_etape_construction">Étape de Construction</label>
                                <select className="border border-gray-300 rounded-md py-2 px-4 mb-3 w-5/6" name="etape_construction" id="id_etape_construction" value={constr} onChange={handleConstrChange}>
                                <option></option>
                                    <option value="1">Acquisition</option>
                                    <option value="2">Étude</option>
                            ...
                            <label className="block text-gray-700" htmlFor="montant_estime">Montant Estimé</label>
                            <input className="border border-gray-300 rounded-md py-2 px-4 mb-3 w-5/6" type="text" name="montant_estime" id="montant_estime" value={formData.montant_estime} onChange={handleChange} />

                            <label className="block text-gray-700" htmlFor="partenariats">Partenariats</label>
                            <input className="border border-gray-300 rounded-md py-2 px-4 mb-3 w-5/6" type="text" name="partenariats" id="partenariats" value={formData.partenariats} onChange={handleChange} />

                            <label className="block text-gray-700" htmlFor="budget_partenaire">Budget Partenaire</label>
                            <input className="border border-gray-300 rounded-md py-2 px-4 mb-3 w-5/6" type="text" name="budget_partenaire" id="budget_partenaire" value={formData.budget_partenaire} onChange={handleChange} />
