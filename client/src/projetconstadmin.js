...
const[prov,setProv]=useState('');
  ...
  const handleDelete = (id) => {
    axios.post('http://localhost:5000/api/modifier', {  id:id})
      .then(response => {
   ...
   const handleDirectionChange = (e) => {
    ...
    setdirection(value);
     axios.post('http://localhost:5000/api/filtrer', { direction: value, datedebut: datedebut, datefin: datefin })
    .then(response => {
  ...
  const handleDateDebutChange = (e) => {
    const value = e.target.value;
    setdatedebut(value);
    axios.post('http://localhost:5000/api/filtrer', { direction: direction,datedebut: datedebut,datefin: datefin })
    .then(response => {
      console.log('Success:', response.data);
      setMessage(response.data.results); 
    })
