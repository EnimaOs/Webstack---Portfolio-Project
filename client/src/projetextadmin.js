...
const[prov,setProv]=useState('');
  ...
  const handleDirectionChange = (e) => {
    ...
    setdirection(value);
    axios.post('http://localhost:3011/api/filtrerextension', { direction: value, datedebut: datedebut, datefin: datefin })
    .then(response => {
  ...
  const handleDateDebutChange = (e) => {
    const value = e.target.value;
    setdatedebut(value);
    axios.post('http://localhost:3011/api/filtrerextension', { direction: direction,datedebut: datedebut,datefin: datefin })
    .then(response => {
      console.log('Success:', response.data);
      setMessage(response.data.results); 
    })
  ...
  const handleDelete = (id,etat) => {
    ...
    axios.post('http://localhost:3011/modifierextension',{id:id})
      .then(response => {
...
<div className="ml-64 flex flex-col justify-center items-center relative top-80">
  ...
  <div className="w-3/4 shadow-lg rounded-lg overflow-hidden mx-auto">
        ...
        {message && message.map((rowData, index) => (
          <tr className={`text-center ${index % 2 === 0 ? 'bg-gray-100' : 'bg-gray-50'}`} key={index}>
            ...
            <td className="px-4 py-2">
              <button onClick={() => handleDelete(rowData.id_projet_extension)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-2">Valider</button>
            </td>
          </tr>
        ))}
      </tbody>
